...
- 训练前症状自查和卒中警示症状分流
- 今日康复复盘：回读上次完成反馈，并依据不适、疲劳、当前疼痛、处方状态和可用时间调整建议优先级
- 每次复盘提供至少三个安全候选、收益与权衡、人工确认，以及可真实下载的交接单
...
测试覆盖反馈闭环与因果排序、移动端 44px 触控和横向溢出、真实交接单下载、AI 输出治理、隐私授权与审计、报告导出、离线队列、冲突合并、账号权限和空数据初始化。`check:recovery-web` 需要本机安装 Chrome 或 Edge。
